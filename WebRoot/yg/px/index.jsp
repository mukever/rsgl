...
function top(){
   	form3.action="<%=basePath%>yg/px/index.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%>yg/px/index.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>yg/px/index.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>yg/px/index.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>yg/px/index.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>yg/px/index.jsp?page="+page;
      form3.submit();
      ...
      <TR>
        ...
        <TD style="PADDING-RIGHT: 10px; PADDING-LEFT: 10px; PADDING-BOTTOM: 10px; COLOR: #566984; PADDING-TOP: 10px; BACKGROUND-COLOR: white" vAlign=top align=middle>
          <DIV>
			...
			int cou = cb.getMessageCount("select count(*) from px  where yg ='"+xm+"'");//得到信息总数			        
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from px  where yg ='"+xm+"' order by id desc",6); //带进一个页数，并返回该页所要显示的信息 
