...
function top(){
   	form3.action="<%=basePath%><%=dir %>/dy/index.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%><%=dir %>/dy/index.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%><%=dir %>/dy/index.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%><%=dir %>/dy/index.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%><%=dir %>/dy/index.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%><%=dir %>/dy/index.jsp?page="+page;
      form3.submit();
  ...
  <TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%" border=0>
    <TBODY>
      <TR style="BACKGROUND-IMAGE: url(<%=basePath %>admin/images/bg_header.gif); BACKGROUND-REPEAT: repeat-x" height=47>
        ...
        HEIGHT: 47px; TEXT-ALIGN: center; 0px: ">待遇管理 </SPAN>
      ...
      <TR>
        ...
        <TD style="PADDING-RIGHT: 10px; PADDING-LEFT: 10px; PADDING-BOTTOM: 10px; COLOR: #566984; PADDING-TOP: 10px; BACKGROUND-COLOR: white" vAlign=top align=middle>
          <DIV>
			...
			int cou = cb.getMessageCount("select count(*) from dy ");//得到信息总数			        
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from dy order by id desc",8); //带进一个页数，并返回该页所要显示的信息 
			...
			%>
                <TR>
                  ...
                  <TD class=gridViewItem><a href="<%=basePath %>admin/dy/add.jsp?method=updy&id=<%=pagelist2.get(0).toString()%>">修改</a></TD>
                  <TD class=gridViewItem><a href="<%=basePath %>ComAction.do?method=deldy&id=<%=pagelist2.get(0).toString()%>">删除</a></TD>
                </TR>
                <%}%>	
