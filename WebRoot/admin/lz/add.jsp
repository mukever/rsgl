...
{//String xm="";String sj="";String yy="";String bz=""; 
	if(document.form1.xm.value==""||document.form1.sj.value==""
	||document.form1.yy.value==""||document.form1.bz.value=="")
	{
		alert("所有内容必须填写！");
...
<%
	...
	String id="";
	String xm="";String sj="";String yy="";String bz=""; 
	if(method.equals("upLZ")){
		id=request.getParameter("id");
		List list=cb.get1Com("select * from lz where id='"+id+"'",5);
		xm=list.get(1).toString();
		sj=list.get(2).toString();
		yy=list.get(3).toString();
		bz=list.get(4).toString(); 
	}	 
...
<BODY   scroll="yes">
	   <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 style="WIDTH: 100%; BORDER-COLLAPSE: collapse" cellSpacing=0 rules=all border=1>
		...
		<form name="form1" action="<%=basePath %>ComAction.do" method="post" onsubmit="return check()">
		  ...
		  <TR  align="center"  >
			<TD width=40%   class=gridViewItem align=right>离职时间：</TD>
			<TD align=left><input type=text size=30 maxlength=50 name=sj value=<%=sj %>></TD>
		  </TR>
		  <TR  align="center"  >
			<TD width=40%   class=gridViewItem align=right>离职原因：</TD>
			<TD align=left><input type=text size=40 maxlength=50 name=yy value=<%=yy %>></TD>
		  </TR>
		   <TR  align="center"  >
