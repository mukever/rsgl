...
function top(){
   	form3.action="<%=basePath%><%=dir %>/px/index.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%><%=dir %>/px/index.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%><%=dir %>/px/index.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%><%=dir %>/px/index.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%><%=dir %>/px/index.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%><%=dir %>/px/index.jsp?page="+page;
      form3.submit();
  ...
  <TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%" border=0>
    <TBODY>
      <TR style="BACKGROUND-IMAGE: url(<%=basePath %>admin/images/bg_header.gif); BACKGROUND-REPEAT: repeat-x" height=47>
        ...
        HEIGHT: 47px; TEXT-ALIGN: center; 0px: ">培训信息管理 </SPAN>
      ...
      <TR>
        ...
        <TD style="PADDING-RIGHT: 10px; PADDING-LEFT: 10px; PADDING-BOTTOM: 10px; COLOR: #566984; PADDING-TOP: 10px; BACKGROUND-COLOR: white" vAlign=top align=middle>
          <DIV>
            ...
            <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 style="WIDTH: 100%; BORDER-COLLAPSE: collapse" cellSpacing=0 rules=all border=1>
              <TBODY>
                <TR>
                  ...
                  <TH class=gridViewHeader scope=col>培训项目名称</TH> 
                  <TH class=gridViewHeader scope=col>培训员工</TH>
			...
			int cou = cb.getMessageCount("select count(*) from px ");//得到信息总数			        
			...
			List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from px order by id desc",6); //带进一个页数，并返回该页所要显示的信息 
			...
			%>
                <TR>
                  ...
                  <TD class=gridViewItem><a href="<%=basePath %>admin/px/add.jsp?method=upPX&id=<%=pagelist2.get(0).toString()%>">修改</a></TD>
                  <TD class=gridViewItem><a href="<%=basePath %>ComAction.do?method=delPX&id=<%=pagelist2.get(0).toString()%>">删除</a></TD>
                </TR>
                <%}%>	
